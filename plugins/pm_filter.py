...
NORGE_IMG = f"https://telegra.ph/file/fef78a892c5fd8e39f09d.jpg"
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(f"Hey {query.from_user.first_name}彡★  ᴛʜɪs ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ....😎", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton("☞𝙲𝙷𝙴𝙲𝙺 𝙿𝙼",url="https://t.me/MovieSearchingBot")]
        )
        btn.insert(0,
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("Hey Bro, നിനക്ക് ഈ സിനിമ വേണമെങ്കിൽ നീ തന്നെ റിക്വസ്റ്റ് ചെയ്യണം🤦‍♂\n\nʀᴇǫᴜᴇsᴛ ʏᴏᴜʀ ᴏᴡɴ 💕", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('💬 𝐆𝐫𝐨𝐮𝐩 💬', url='https://t.me/cinema_collections ')
         ]]
        try:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ɢʀᴏᴜᴘ', url='https://t.me/cinema_collections')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            hrr = await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(600)
            await hrr.delete()
            await message.delete()
        except Exception as e:
        ...
        ho = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(600)
        await ho.delete()
        await message.delete()
    if spoll:
